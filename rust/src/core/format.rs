...
pub fn format_session_table(
    ...
    status_map: Option<&HashMap<String, RunStatus>>,
) -> String {
...
pub fn format_bookmark_table(bookmarks: &[Bookmark]) -> String {
    ...
    for b in bookmarks {
        ...
        let category = if b.category.is_empty() {
            "-".to_string()
        } else {
            b.category.clone()
        };
...
pub fn format_space_tree(spaces: &[Space], bookmarks: &[Bookmark]) -> String {
    ...
    fn render_node(
        ...
        let child_len = children_map
            .get(&Some(space.id.clone()))
            .map(|v| v.len())
            .unwrap_or(0);
...
mod tests {
    ...
    fn space_tree_without_bookmarks_shows_catalogs_only() {
        let spaces = vec![
            space("root", "Research", None),
            space("child", "Paper", Some("root")),
        ];
