...
pub fn runs_path() -> PathBuf {
    std::env::var("STARLING_RUNS").map(PathBuf::from).unwrap_or_else(|_| default_runs_path())
}

fn empty_runs() -> RunsFile {
    RunsFile { version: RUNS_VERSION, runs: Vec::new() }
}

pub fn load_runs() -> RunsFile {
    let data: Option<RunsFile> = read_json(&runs_path());
    match data {
        Some(d) => RunsFile { version: RUNS_VERSION, runs: d.runs },
        None => empty_runs(),
    }
}
...
pub fn save_runs(mut data: RunsFile) {
    if data.runs.len() > MAX_RUN_RECORDS {
        let mut running: Vec<RunRecord> = data.runs.iter()
            .filter(|r| r.status == RunStatus::Running).cloned().collect();
        let mut terminal: Vec<RunRecord> = data.runs.iter()
            .filter(|r| r.status != RunStatus::Running).cloned().collect();
...
pub fn remove_run(run_id: &str) -> bool {
    ...
    if data.runs.len() == before { return false; }
    save_runs(data);
    true
}
    ...
    data.runs.retain(|r| {
        if let Some(f) = &filter {
            if let Some(sid) = &f.session_id {
                if r.session_id.as_ref() != Some(sid) { return true; }
            }
...
pub fn find_runs_by_session(session_id: &str) -> Vec<RunRecord> {
    let mut runs: Vec<RunRecord> = load_runs().runs.into_iter()
...
pub fn list_runs(filter: Option<ListFilter>) -> Vec<RunRecord> {
    ...
    if let Some(f) = filter {
        if let Some(s) = f.status { runs.retain(|r| r.status == s); }
...
pub fn status_badge(status: RunStatus, use_color: bool) -> String {
    ...
    if !use_color { return glyph.to_string(); }
    match status {
...
pub fn summarize_run_status(bookmarks: &[Bookmark], color: bool) -> String {
    ...
    let parts: Vec<String> = RUN_STATUS_ORDER.iter()
        .filter_map(|s| counts.get(s).map(|n| format!("{}{}", status_badge(*s, color), n)))
        .collect();
    if parts.is_empty() {
...
pub fn detect_running_sessions() -> HashMap<String, DetectedSession> {
    ...
    for (session_id, info) in mapped {
        detected.insert(session_id, DetectedSession {
            pid: if info.pid > 0 { Some(info.pid) } else { None },
            provider: info.provider.map(|p| match p {
...
mod tests {
    ...
    fn mk_run(run_id: &str, session_id: Option<&str>, status: RunStatus, started: &str) -> RunRecord {
        RunRecord {
            run_id: run_id.into(),
    ...
    fn lifecycle_create_finalize_remove() {
        with_temp_store(|| {
            std::env::set_var("STARLING_RUNS", "/tmp/starling-runs-test-does-not-exist-yet.json");
    ...
    fn list_runs_sorts_newest_first() {
        with_temp_store(|| {
            ...
            create_run(mk_run("old", Some("s1"), RunStatus::Completed, "2026-01-01T00:00:00Z"));
            create_run(mk_run("new", Some("s1"), RunStatus::Completed, "2026-02-01T00:00:00Z"));
    ...
    fn summarize_counts_bookmarks() {
        let b = vec![
            Bookmark {
                ...
                session_id: "s1".into(), title: "t".into(), category: "c".into(),
                tags: vec![], project_path: "/p".into(), first_prompt: "".into(),
