...
pub fn load_store() -> Store {
    ...
    let mut legacy_map: std::collections::HashMap<String, String> = std::collections::HashMap::new();
    let used_cat_ids: std::collections::HashSet<String> = store.spaces.iter()
    ...
    if !store.spaces.iter().any(|s| s.name == "claude") {
        ...
        store.spaces.push(Space {
            id, name: "claude".into(), description: "Claude Code sessions".into(),
            tags: vec![], parent_id: None, created_at: now.clone(), updated_at: now,
        });
        migrated = true;
    }
    if !store.spaces.iter().any(|s| s.name == "codex") {
        ...
        let id = generate_space_id(&store.spaces);
        store.spaces.push(Space {
            id, name: "codex".into(), description: "Codex sessions".into(),
            tags: vec![], parent_id: None, created_at: now.clone(), updated_at: now,
        });
        migrated = true;
    }
...
fn apply_patch(source: &Bookmark, patch: BookmarkPatch) -> Bookmark {
    Bookmark {
        ...
        project_path: patch.project_path.unwrap_or_else(|| source.project_path.clone()),
        first_prompt: patch.first_prompt.unwrap_or_else(|| source.first_prompt.clone()),
...
pub fn list_bookmarks(filter: BookmarkFilter) -> Vec<Bookmark> {
    ...
    store.bookmarks.into_iter()
        .filter(|b| filter.category.as_ref().map(|c| &b.category == c).unwrap_or(true))
        .filter(|b| filter.tag.as_ref().map(|t| b.tags.contains(t)).unwrap_or(true))
        .collect()
}
    ...
    load_store().bookmarks.into_iter()
        .filter(|b| {
            b.title.to_lowercase().contains(&q)
                ...
                || b.notes.iter().any(|n| n.content.to_lowercase().contains(&q))
        })
        .collect()
}
...
pub fn find_space(id_or_name: &str) -> Option<Space> {
    ...
    if matches.len() == 1 { Some(matches.into_iter().next().unwrap()) } else { None }
}
...
pub fn find_space_candidates(id_name_or_path: &str) -> Vec<Space> {
    let store = load_store();
    if let Some(exact) = store.spaces.iter().find(|s| s.id == id_name_or_path).cloned() {
        return vec![exact];
    }
    if id_name_or_path.contains('/') {
        return find_space_path_candidates(id_name_or_path, &store.spaces);
    }
    store.spaces.into_iter().filter(|s| s.name == id_name_or_path).collect()
}

fn find_space_path_candidates(path_ref: &str, spaces: &[Space]) -> Vec<Space> {
    let parts: Vec<&str> = path_ref.split('/').map(|p| p.trim()).filter(|p| !p.is_empty()).collect();
    if parts.is_empty() { return vec![]; }

    let mut candidates: Vec<Space> = spaces.iter()
        .filter(|s| s.name == parts[0] && s.parent_id.is_none())
        .cloned()
        .collect();
    for part in parts.iter().skip(1) {
        let parent_ids: std::collections::HashSet<String> = candidates.iter().map(|s| s.id.clone()).collect();
        candidates = spaces.iter()
            .filter(|s| s.name == *part && s.parent_id.is_some() && s.parent_id.as_ref().map(|p| parent_ids.contains(p)).unwrap_or(false))
            .cloned()
            .collect();
...
pub fn has_sibling_space_name(name: &str, parent_id: Option<&str>, exclude_id: Option<&str>) -> bool {
    load_store().spaces.iter().any(|s| {
        s.name == name
            && s.parent_id.as_deref() == parent_id
...
pub fn update_space(id: &str, patch: SpacePatch) -> Option<Space> {
    let mut store = load_store();
    let idx = store.spaces.iter().position(|s| s.id == id || s.name == id)?;
...
fn apply_space_patch(source: &Space, patch: SpacePatch) -> Space {
    Space {
        ...
        description: patch.description.unwrap_or_else(|| source.description.clone()),
        tags: patch.tags.unwrap_or_else(|| source.tags.clone()),
...
mod tests {
    ...
    fn add_and_find_bookmark() {
        with_temp_store(|| {
            let b = Bookmark {
                id: "starling_0001".into(), provider: "claude".into(),
                session_id: "sess-abc".into(), title: "Hello".into(),
                category: "demo".into(), tags: vec!["x".into()],
                project_path: "/p".into(), first_prompt: "hi".into(),
                notes: vec![], space_ids: vec!["cat_0001".into()],
    ...
    fn update_and_remove_bookmark() {
        with_temp_store(|| {
            let b = Bookmark {
                id: "starling_0001".into(), provider: "claude".into(),
                session_id: "sess-xyz".into(), title: "Old".into(),
                category: "a".into(), tags: vec![],
                project_path: "/p".into(), first_prompt: "".into(),
                notes: vec![], space_ids: vec![],
                created_at: now_iso(), updated_at: now_iso(),
            };
            add_bookmark(b);
            let patch = BookmarkPatch { title: Some("New".into()), ..Default::default() };
    ...
    fn space_subtree_removal_unlinks_bookmarks() {
        with_temp_store(|| {
            let parent = Space {
                id: "cat_0099".into(), name: "parent".into(), description: "".into(),
                tags: vec![], parent_id: None, created_at: now_iso(), updated_at: now_iso(),
            };
            add_space(parent);
            let child = Space {
                id: "cat_0100".into(), name: "child".into(), description: "".into(),
                tags: vec![], parent_id: Some("cat_0099".into()),
                created_at: now_iso(), updated_at: now_iso(),
            };
            add_space(child);
            let b = Bookmark {
                id: "starling_0001".into(), provider: "claude".into(),
                session_id: "s1".into(), title: "T".into(),
                category: "c".into(), tags: vec![],
                project_path: "/p".into(), first_prompt: "".into(),
                notes: vec![], space_ids: vec!["cat_0099".into(), "cat_0100".into()],
                created_at: now_iso(), updated_at: now_iso(),
            };
            add_bookmark(b);
            assert!(remove_space("cat_0099"));
            let b = find_bookmark("s1").unwrap();
            assert!(b.space_ids.is_empty(), "space_ids should be cleared: {:?}", b.space_ids);
    ...
    fn find_space_path_resolution() {
        with_temp_store(|| {
            let root = Space {
                id: "cat_0101".into(), name: "root".into(), description: "".into(),
                tags: vec![], parent_id: None, created_at: now_iso(), updated_at: now_iso(),
            };
            add_space(root);
            let child = Space {
                id: "cat_0102".into(), name: "leaf".into(), description: "".into(),
                tags: vec![], parent_id: Some("cat_0101".into()),
                created_at: now_iso(), updated_at: now_iso(),
            };
            add_space(child);
