...
pub fn handle(cmd: SessionCommand) -> Result<()> {
    match cmd {
        ...
        SessionCommand::Meta {
            ...
            add_tags,
            json,
        } => meta_cmd(&session_id, title, tags, add_tags, json),
        SessionCommand::Note {
            session_id,
            content,
            json,
        } => note_cmd(&session_id, &content.join(" "), json),
...
fn build_status_map(sessions: &[SessionMeta]) -> HashMap<String, RunStatus> {
    sessions
        ...
        .map(|s| {
            (
                s.session_id.clone(),
                get_run_status_for_session(&s.session_id),
            )
        })
        .collect()
}
    ...
    if let Some(c) = catalog_filter {
        match resolve_catalog_reference(c) {
            crate::core::catalog_resolver::CatalogResolution::Found(s) => {
                target_space_ids = Some(vec![s.id])
            }
...
fn lookup_cmd(session_ids: Vec<String>, agent: Option<String>, json: bool) -> Result<()> {
    ...
    if json {
        ...
        if resolved < requested {
            eprintln!(
                "{}",
                format!("Resolved {}/{} sessions.", resolved, requested).normal()
            );
        }
        return Ok(());
    }
    if sessions.is_empty() {
        println!(
            "{}",
            format!("No sessions found for {} id(s).", requested).yellow()
        );
        return Ok(());
    }
...
fn meta_cmd(
    ...
    if patch.title.is_none() && patch.tags.is_none() {
        ...
        println!(
            "{}",
            format!("No metadata changes provided for {}.", bookmark.id).yellow()
        );
        return Ok(());
    }
    ...
    println!(
        "{}",
        format!("Updated session metadata: {}", updated.id).green()
    );
    Ok(())
}
...
fn unpin_cmd(session_id: &str, json: bool) -> Result<()> {
    let bookmark = match find_bookmark(session_id) {
        ...
        None => {
            ...
            println!(
                "{}",
                format!("Session metadata not found: {}", session_id).yellow()
            );
            return Ok(());
        }
    };
    ...
    if json {
        return super::print_json_result(
            "session.unpin",
            &format!(
                "Removed pin metadata for {}",
                short_session_id(&bookmark.session_id)
            ),
...
fn delete_cmd(session_id: &str, yes: bool, json: bool) -> Result<()> {
    ...
    println!(
        "{}",
        format!("Deleted session {}", short_session_id(&meta.session_id)).green()
    );
    println!("  File: {}", meta.file_path);
    if let Some(b) = &bookmark {
    ...
    match sub {
        ...
        IndexCommand::Clear { json } => {
            ...
            println!(
                "{}",
                if removed {
                    "Session index removed.".green()
                } else {
                    "No session index found.".yellow()
                }
            );
            Ok(())
        }
    ...
    match sub {
        SessionCatalogCommand::Add {
            ...
            if bookmark.space_ids.contains(&entry.id) {
                ...
                println!(
                    "{}",
                    format!("Session already in catalog \"{}\".", entry.name).yellow()
                );
                return Ok(());
            }
            ...
            if json {
                ...
                return super::print_json_result(
                    "session.catalog.add",
                    &format!(
                        "Added session {} to catalog \"{}\"",
                        short_session_id(&updated.session_id),
                        entry.name
                    ),
        ...
        SessionCatalogCommand::Remove {
            ...
            if json {
                let updated =
                    find_bookmark(&bookmark.session_id).unwrap_or_else(|| bookmark.clone());
                return super::print_json_result(
                    "session.catalog.remove",
                    &format!(
                        "Removed session {} from catalog \"{}\"",
                        short_session_id(&bookmark.session_id),
                        entry.name
                    ),
