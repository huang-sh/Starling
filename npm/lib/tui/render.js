...
export function renderStarlingFrame(state, viewport) {
    ...
    return lines
        .map((line) => colorize(fitTerminalLine(line.text, width), line.tone, viewport.color === true, line.unstyled, line.link))
        .join("\n");
}
function styleLines(lines, width, color) {
    return lines.map((line) => colorize(fitTerminalLine(line.text, width), line.tone, color, line.unstyled, line.link));
}
function buildTranscriptRecords(state) {
    return [
 ...
 */
function renderTranscriptSplit(state, width) {
    ...
    while (split > 0) {
        ...
        if (tail.kind === "timeline" ? tail.value.pending !== true : false)
            break;
        split -= 1;
    }
    return {
 ...
 */
export function renderStarlingParts(state, viewport) {
    const width = Math.max(1, Math.floor(viewport.width));
    const height = Math.max(1, Math.floor(viewport.height));
    const tick = Math.max(0, Math.floor(viewport.tick ?? 0));
    ...
    const footer = renderFooter(state, width, height, tick);
    const overlay = Boolean(state.uiPrompt
        || state.modelPicker
        || state.treePicker
    ...
    if (liveTail.length > 0)
        live.push(...liveTail);
    if (liveTail.length > 0 && footer.length > 0)
        live.push({ text: "" });
    if (footer.length > 0)
        live.push(...footer);
    return {
...
function renderCompactFrame(state, width, height, tick, color) {
    if (state.uiPrompt) {
        return renderInteraction(state, width, height, tick)
            .slice(-height)
            .map((line) => colorize(fitTerminalLine(line.text, width), line.tone, color, line.unstyled, line.link))
            .join("\n");
    }
    if (state.authPicker) {
    ...
    const compactInput = wrapEditorLines(state.composer, state.composerCursor, Math.max(1, width - 2), 1).at(-1) ?? { text: "▏", hasCursor: true, cursorIndex: 0 };
    lines.push({
...
function renderFooter(state, width, height, tick) {
    if (state.uiPrompt)
        return renderInteraction(state, width, height, tick);
    if (state.authPicker)
        return renderAuthPicker(state.authPicker, width, height, tick, state.statusItems.auth);
...
function renderAuthPicker(picker, width, height, tick, authStatusText) {
    ...
    const start = Math.max(0, Math.min(activeIndex - Math.floor(listLimit / 2), providers.length - listLimit));
...
function renderTreePicker(picker, width, height, tick) {
    ...
    if (picker.stage === "custom") {
        ...
        const input = wrapEditorLines(picker.customPrompt, picker.customPrompt.length, editorWidth, Math.max(2, Math.min(8, height - 7)));
        const rows = [
    ...
    const start = Math.max(0, Math.min(activeIndex - Math.floor(listLimit / 2), entries.length - listLimit));
    if (entries.length === 0) {
...
function renderModelPicker(picker, width, height) {
    ...
    const start = Math.max(0, Math.min(selected - Math.floor(listLimit / 2), models.length - listLimit));
...
function renderModelProviderTabs(picker, width) {
    const chunks = [
    ...
    for (const [index, chunk] of chunks.entries()) {
        ...
        if (text && visibleWidth(text + separator + chunk.text) > width)
            flush();
        if (!text && index > 0)
            text = "  ";
        else
        ...
        text += chunk.text;
        if (!chunk.active)
            unstyled.push([start, text.length]);
    }
    flush();
    return rows;
}
...
function renderExtensionStatus(state, width) {
    const rows = [];
...
function renderExtensionWidgets(state, placement, width) {
    const rows = [];
...
function renderInteraction(state, width, height, tick) {
    ...
    if (prompt.method === "confirm" || prompt.method === "select") {
        ...
        const inputLines = wrapEditorLines(editor.value, editor.cursor, Math.max(1, width - 6), 5);
...
function maskEditorValue(value, cursor) {
...
function editorBottomLine(line, width) {
    const left = "› ";
...
function wrapEditorLines(value, cursor, width, maxLines) {
    const at = editorCursorBoundary(value, cursor ?? value.length);
    const cleanValue = sanitizeTerminalText(value);
    const cleanCursor = Math.min(cleanValue.length, sanitizeTerminalText(value.slice(0, at)).length);
    const marker = editorCursorMarker(cleanValue);
    const wrapped = wrapTerminalText(`${cleanValue.slice(0, cleanCursor)}${marker}${cleanValue.slice(cleanCursor)}`, width);
...
function editorTextRanges(line, offset) {
    if (!line.text)
        return [];
    ...
    if (line.cursorIndex > 0)
        ranges.push([offset, offset + line.cursorIndex]);
...
function editorCursorBoundary(value, cursor) {
    const bounded = Math.min(Math.max(0, Math.trunc(cursor)), value.length);
    let boundary = 0;
    for (const part of graphemeSegmenter.segment(value)) {
        if (part.index > bounded)
            break;
        boundary = part.index;
        if (part.index + part.segment.length <= bounded)
            boundary = part.index + part.segment.length;
    }
    return boundary;
}
function editorCursorMarker(value) {
...
function colorize(value, tone, enabled, unstyled, link) {
    if (!enabled || !tone)
        return value;
