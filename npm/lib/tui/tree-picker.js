...
export function treePickerFromResponse(value) {
    if (!isRecord(value))
    ...
    for (let id = leafId; id; id = parents.get(id) ?? null)
        activePath.add(id);
    const entries = [];
    ...
    while (stack.length > 0) {
        ...
        if (!isRecord(item.node) || !isRecord(item.node.entry))
            continue;
        const entry = item.node.entry;
        const id = string(entry.id);
        if (!id)
            continue;
        const children = activeFirst(Array.isArray(item.node.children) ? item.node.children : [], activePath);
...
function activeFirst(nodes, activePath) {
...
export function visibleTreeEntries(picker) {
...
export function selectedTreeEntry(picker) {
    return visibleTreeEntries(picker)[picker.selected];
}
function collectParents(nodes, output) {
    const stack = [...nodes];
    while (stack.length > 0) {
        ...
        if (id)
            output.set(id, typeof node.entry.parentId === "string" ? node.entry.parentId : null);
...
export function handleTreePickerKey(host, key) {
    ...
    if (picker.stage === "custom") {
        ...
        if (key.type === "enter" || key.type === "ctrl-s") {
            ...
            if (picker.targetId)
                void navigateTree(host, picker.targetId, true, prompt);
            return;
        }
    ...
    if (picker.stage === "summary") {
        ...
        if (key.type === "enter") {
            ...
            if (!action || !picker.targetId)
                return;
            if (action.id === "custom")
                host.dispatch({ type: "tree.custom.open" });
            else
                void navigateTree(host, picker.targetId, action.id === "summary");
        }
        return;
    }
    if (key.type === "escape" || key.type === "ctrl-c") {
...
async function navigateTree(host, targetId, summarize, customInstructions) {
    if (!host.session || !host.state.treePicker || host.state.treePicker.working || host.closing)
        return;
    ...
    try {
        ...
        host.dispatch({
            ...
            event: {
                ...
                snapshot: normalizeChatSnapshot(sessionState, Array.isArray(messagesData.messages) ? messagesData.messages : []),
            },
        });
