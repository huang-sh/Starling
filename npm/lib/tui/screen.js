...
export class StarlingScreen {
    ...
    doRender(parts, height, force) {
        ...
        const computeLineDiff = (targetRow) => (targetRow - viewportTop) - (hardwareCursorRow - prevViewportTop);
    ...
    fullRender(clear, newLines, height) {
        let buffer = this.synchronizedOutput ? SYNC_BEGIN : "";
        if (clear)
 ...
 */
export function shouldUseSynchronizedOutput(environment = process.env) {
    ...
    if (["1", "true", "yes", "on"].includes(explicit ?? ""))
        return true;
    if (environment.TMUX || environment.STY || environment.TERM === "screen")
        return false;
    const terminal = [
    ...
    return Boolean(environment.WT_SESSION
