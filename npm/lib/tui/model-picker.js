...
export function availableModelsFromResponse(value) {
...
export function modelPickerProviders(picker) {
...
export function visibleModelPickerModels(picker) {
...
function modelSearchScore(model, query) {
    ...
    for (const value of values) {
        if (value === query)
            ...
            best = Math.min(best, 10 + value.length - query.length);
        else {
            const contains = value.indexOf(query);
            if (contains >= 0)
                best = Math.min(best, 100 + contains);
            else {
                const fuzzy = subsequenceScore(value, query);
                if (fuzzy !== null)
                    best = Math.min(best, 1_000 + fuzzy);
            }
        }
    }
...
export function handleModelPickerKey(host, key) {
...
async function configureSelectedModel(host) {
    const picker = host.state.modelPicker;
    if (!host.session || !picker || picker.stage !== "thinking" || picker.switching || host.closing)
        return;
