...
export type AuthPickerMode = "login" | "logout";
export type AuthProviderType = "oauth" | "api_key";
...
export interface AuthPickerState {
  mode: AuthPickerMode;
...
export function authProvidersFromResponse(value: unknown): AuthProviderOption[] {
...
export function visibleAuthProviders(picker: AuthPickerState): AuthProviderOption[] {
  const query = picker.query.trim().toLocaleLowerCase();
  if (!query) return picker.providers;
...
export function selectedAuthProvider(picker: AuthPickerState): AuthProviderOption | undefined {
...
export function handleAuthPickerKey(host: PickerHost, key: StarlingKey): void {
  ...
  if (key.type === "enter") {
    ...
    if (provider) void authenticateProvider(host, provider);
    return;
  }
...
export async function authenticateProvider(host: PickerHost, provider: AuthProviderOption): Promise<void> {
  const picker = host.state.authPicker;
  if (!host.session || !picker || picker.working || host.closing) return;
