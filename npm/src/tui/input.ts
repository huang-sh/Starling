...
export class StarlingInputDecoder {
  ...
  flushPendingEscape(): StarlingKey[] {
    ...
    if (this.inPaste) {
      ...
      this.buffer = "";
      this.inPaste = false;
      if (this.paste) keys.push({ type: "paste", value: normalizePaste(this.paste) });
      this.paste = "";
    }
    return keys;
  }

  private decode(flush: boolean): StarlingKey[] {
    const keys: StarlingKey[] = [];
    ...
    while (this.buffer.length > 0 || (flush && this.inPaste)) {
      ...
      if (sequence) {
        if (sequence.key) keys.push(sequence.key);
  ...
  private appendPaste(value: string): void {
    ...
    if (remaining > 0) this.paste += value.slice(0, remaining);
  }
}

/** Parse terminal input without depending on readline or a third-party TUI. */
export function parseStarlingKeys(input: string): StarlingKey[] {
  const decoder = new StarlingInputDecoder();
  return [...decoder.push(input), ...decoder.end()];
}

function appendCodePoint(keys: StarlingKey[], codePoint: number, value: string): void {
...
function keyFromCodePoint(codePoint: number, modifiers: number): StarlingKey | undefined {
  const lower = codePoint >= 0x41 && codePoint <= 0x5a ? codePoint + 0x20 : codePoint;
  if ((modifiers & CTRL_MODIFIER) !== 0) {
    if (lower === 0x63) return { type: "ctrl-c" };
...
function terminalSequenceSuffix(value: string, sequence: string): number {
...
function terminalSequence(value: string): { key?: StarlingKey; length: number } | null {
...
function completeCsiSequenceLength(value: string): number | null {
  if (!value.startsWith(`${ESC}[`) || value.length < 3) return null;
...
function parseCsiKey(sequence: string): StarlingKey | undefined {
  const kitty = sequence.match(
    /^\u001b\[(\d+)(?::(\d*))?(?::(\d+))?(?:;(\d+))?(?::([123]))?u$/,
  );
  if (kitty) {
    if (kitty[5] === "3") return undefined;
  ...
  if (modifyOtherKeys) {
    return keyFromCodePoint(
      Number.parseInt(modifyOtherKeys[2] ?? "", 10),
      Number.parseInt(modifyOtherKeys[1] ?? "1", 10) - 1,
    );
  }
  ...
  if (functional) {
    if (functional[3] === "3") return undefined;
...
function keyFromKittyCodePoint(codePoint: number, modifiers: number): StarlingKey | undefined {
  const equivalent = kittyFunctionalEquivalent(codePoint);
  if (equivalent === undefined || typeof equivalent !== "number") return equivalent;
  const key = keyFromCodePoint(equivalent, modifiers);
  if (key) return key;
  const effectiveModifiers = modifiers & ~(1 | LOCK_MODIFIERS);
  if (effectiveModifiers !== 0 || equivalent < 0x20 || equivalent > 0x10ffff) return undefined;
  if (equivalent >= 0x7f && equivalent <= 0x9f) return undefined;
  return { type: "text", value: String.fromCodePoint(equivalent) };
}

function kittyFunctionalEquivalent(codePoint: number): number | StarlingKey | undefined {
  if (codePoint >= 57399 && codePoint <= 57408) return 0x30 + codePoint - 57399;
  switch (codePoint) {
    case 57409:
      return 0x2e;
    case 57410:
...
function canUseKittyBaseLayout(codePoint: number): boolean {
  const equivalent = kittyFunctionalEquivalent(codePoint);
  if (typeof equivalent !== "number") return false;
  if (equivalent < 0 || equivalent > 0x10ffff) return true;
...
function navigationKey(finalByte: string): StarlingKey | undefined {
  switch (finalByte) {
    case "A":
      return { type: "up" };
    case "B":
...
function isRecognizedCsiSequence(sequence: string): boolean {
 ...
 */
function unknownEscapeSequenceLength(value: string, flush: boolean): number | null {
  if (value === ESC) return flush ? 0 : null;
  if (!flush && TERMINAL_SEQUENCES.some(([sequence]) => sequence.startsWith(value))) return null;
  if (!flush && `${ESC}${ESC}`.startsWith(value)) return null;
  const introducer = value[1];
  if (introducer === ESC) return 1;
  if (introducer === "[") {
    const length = completeCsiSequenceLength(value);
    return length ?? (flush ? value.length : null);
  }
  if (introducer === "O") return value.length >= 3 ? 3 : flush ? value.length : null;
  if (introducer === "]") {
