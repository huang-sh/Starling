...
export interface ZhipuModelRuntime {
  getModels?(providerId?: string): readonly unknown[] | Promise<readonly unknown[]>;
...
export async function registerZhipuCodingPlanProvider(
  runtime: ZhipuModelRuntime,
  environment: NodeJS.ProcessEnv,
  fetchImpl: ProviderFetch = (input, init) => fetch(input, init),
): Promise<void> {
  if (!runtime.registerProvider) return;
...
function dynamicModel(
  entry: unknown,
  known: Map<string, ProviderModel>,
...
async function fallbackModels(runtime: ZhipuModelRuntime): Promise<ProviderModel[]> {
  const source = await runtime.getModels?.("zai-coding-cn") ?? [];
  const inherited = source
    .map(normalizePiModel)
    .filter((model): model is ProviderModel => model !== undefined);
  if (inherited.length) return uniqueModels(inherited);
...
function normalizePiModel(value: unknown): ProviderModel | undefined {
  if (!isJsonObject(value) || typeof value.id !== "string" || !value.id.trim()) return undefined;
  ...
  if (isJsonObject(value.cost)) model.cost = value.cost;
  if (isJsonObject(value.thinkingLevelMap)) model.thinkingLevelMap = value.thinkingLevelMap;
  if (isJsonObject(value.compat)) model.compat = value.compat;
  return model;
}
...
interface ProviderModel extends Record<string, unknown> {
  id: string;
  name: string;
  reasoning: boolean;
  input: ("text" | "image")[];
