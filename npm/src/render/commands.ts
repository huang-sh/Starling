...
export function getRenderPlan(args: string[]): RenderPlan | null {
  if (args.length === 0 || hasMachineOrHelpFlag(args)) return null;
...
export function renderCommandResult(plan: RenderPlan, raw: unknown): string {
  switch (plan.kind) {
    case "sessionList":
...
function planSession(rest: string[]): RenderPlan | null {
...
function planCatalog(rest: string[]): RenderPlan | null {
  const sub = normalizeSub(rest[0], "list");
  if (sub === "list") {
...
function planProject(rest: string[]): RenderPlan | null {
  const sub = normalizeSub(rest[0], "list");
  if (sub === "list") {
...
function planModel(rest: string[]): RenderPlan | null {
  const sub = normalizeSub(rest[0], "list");
  if (sub === "list") {
...
function planConfig(rest: string[]): RenderPlan | null {
...
function planStatus(rest: string[]): RenderPlan | null {
  const sub = normalizeSub(rest[0], "show");
  if (sub === "show" || sub === "list") {
...
function planRun(rest: string[]): RenderPlan | null {
...
function renderSessionList(rows: unknown[]): string {
  if (rows.length === 0) return ansi.yellow("No sessions found.");
  return renderTable(
    "Sessions",
    rows.map((row) => {
    ...
    ["Session", "Agent", "Model", "Project", "Modified", "Input", "Output", "Cache", "Total"],
    true,
    [15, 8, 18, 40, 16, 9, 9, 9, 9],
  );
}

function renderSessionShow(row: Record<string, unknown>): string {
  if (!row.session_id) return ansi.yellow("Session not found.");
...
function renderCatalogList(rows: unknown[]): string {
  if (rows.length === 0) return ansi.yellow("No catalogs created yet.");
  ...
  return renderTable(
    ...
    ["Catalog ID", "Name", "Sessions", "Pins", "Parent", "Description", "Tags"],
    true,
    [18, 20, 8, 6, 16, 30, 22],
  );
}

function renderCatalogTree(rows: unknown[], includeSessions: boolean): string {
  const spaces = rows.map(asRecord);
  if (spaces.length === 0) return ansi.yellow("No catalogs created yet.");
  ...
  const lines: string[] = [ansi.bold("starling")];
...
function renderCatalogShow(rows: unknown[], query: string): string {
  const spaces = rows.map(asRecord);
  const found = resolveCatalog(spaces, query);
  if (!found) return ansi.yellow(`Catalog not found: ${query}`);
...
function renderProjectList(rows: unknown[]): string {
  if (rows.length === 0) return ansi.yellow("No projects found.");
  return renderTable(
    "Projects",
    rows.map((row) => {
      const r = asRecord(row);
    ...
    ["PROJECT", "SESSIONS", "AGENTS", "TOP MODEL", "LAST ACTIVE"],
    true,
    [48, 8, 26, 22, 16],
  );
}

function renderProjectShow(row: Record<string, unknown>): string {
  const sessions = Array.isArray(row.sessions) ? row.sessions.map(asRecord) : [];
  if (!row.project_path && sessions.length === 0) return ansi.yellow("No sessions for project.");
  const lines = [
  ...
  if (sessions.length) {
    ...
    lines.push(renderTable(
      ...
      ["Session", "Agent", "Model", "Modified", "Prompt"],
      false,
      [15, 8, 18, 16, 46],
    ));
  }
...
function renderModelList(rows: unknown[]): string {
  if (rows.length === 0) return ansi.yellow("No model configurations found.");
...
function renderStatusList(rows: unknown[]): string {
  if (rows.length === 0) return ansi.yellow("No cataloged sessions found.");
  return renderTable(
    "Status",
    rows.map((row) => {
      const r = asRecord(row);
    ...
    ["Catalog", "Session", "Title", "Status", "Started", "Ended", "Exit", "PID"],
    true,
    [22, 15, 48, 11, 16, 16, 6, 8],
  );
}

function renderRunList(rows: unknown[]): string {
  if (rows.length === 0) return ansi.yellow("No runs recorded.");
  return renderTable(
    "Runs",
    rows.map((row) => {
      const r = asRecord(row);
    ...
    ["Run", "Agent", "Status", "Started", "Ended", "Exit", "PID", "Project"],
    true,
    [15, 8, 11, 16, 16, 6, 8, 40],
  );
}

function renderRunShow(row: Record<string, unknown>): string {
  if (!row.run_id) return ansi.yellow("Run not found.");
  return [
...
function renderMutationResult(row: Record<string, unknown>): string {
...
function renderKeyValue(title: string, row: Record<string, unknown>): string {
  const keys = Object.keys(row);
  if (keys.length === 0) return ansi.yellow(`${title}: empty`);
  const entries = title === "Starling config" ? configEntries(row) : keys.map((key) => [labelFor(key), formatValue(row[key])] as const);
  return [
    ansi.bold(title),
...
function renderTable(title: string, rows: string[][], headers: string[], includeTitle = true, widths?: number[]): string {
  const lines: string[] = [];
...
function detailLine(label: string, value: string, indent = 2): string {
...
function boxRow(values: string[], widths: number[]): string {
...
function inferWidth(index: number, header: string, rows: string[][]): number {
  const longest = Math.max(visible(header), ...rows.map((row) => visible(row[index] ?? "")));
  return Math.min(Math.max(longest, 6), 36);
}

function resolveCatalog(spaces: Record<string, unknown>[], query: string): Record<string, unknown> | undefined {
...
function parentName(space: Record<string, unknown>, all: Record<string, unknown>[]): string {
...
function withPins(args: string[]): string[] {
  return args.includes("--pins") ? args : [...args, "--pins"];
}

function hasMachineOrHelpFlag(args: string[]): boolean {
  return args.some((arg) => arg === "--json" || arg === "-h" || arg === "--help" || arg === "help");
}

function normalizeRoot(value: string): string {
  if (value === "s" || value === "ses") return "session";
  if (value === "cat" || value === "space" || value === "sp") return "catalog";
  if (value === "prj") return "project";
  if (value === "models") return "model";
  return value;
}

function normalizeSub(value: string | undefined, fallback: string): string {
...
function formatAgentModelSummary(value: unknown): string {
...
function topCountKey(value: unknown): string {
...
function cacheTokens(usage: Record<string, unknown>): number {
...
function profileName(row: Record<string, unknown>): string {
  const name = str(row.name) || "-";
  const scope = str(row.scope);
  if (name === "current" && scope === "current") return "default";
  return name;
}
...
function configEntries(row: Record<string, unknown>): readonly (readonly [string, string])[] {
  return [
