...
// --- Read-only batch lookup (fast path for catalog expand) ---

const LOOKUP_FRESH_TTL_MS = 60_000;

/**
 * Cheap freshness check for read-only lookups. Stats ONLY the top-level
 * session roots (no recursion) and combines with a short TTL floor so that
 * repeated expands do not hammer the filesystem. Intentionally does not use
 * isSessionIndexStale / newestMtimeInTree, which recurse the whole tree.
 */
export function isSessionIndexFresh(provider?: Provider, now: number = Date.now()): boolean {
  const index = loadSessionIndex();
  if (!index) return false;
  ...
  for (const root of roots) {
    try {
      ...
      // Root missing/unreadable -> can't prove freshness, fall back to slow path.
      return false;
    }
  }
  return true;
...
/**
 * Batch read-only lookup. On the fast path (index fresh) this only reads the
 * on-disk index and filters in memory: no readdirSync, no jsonl parse, no
 * writeSessionIndex. On the slow path it refreshes once via
 * loadSessionIndexWithNewFiles (one walk + one write regardless of input size).
 *
 * Result is keyed by the canonical session_id from the index. Prefix inputs are
 * supported via matchesSessionId; exact id wins over prefix.
 */
export async function lookupIndexedSessions(
  sessionIds: string[],
  provider?: Provider
  ...
  const index = isSessionIndexFresh(provider)
    ? loadSessionIndex()
    : await loadSessionIndexWithNewFiles(provider);
  if (!index) return result;

  for (const session of index.sessions) {
    if (provider && session.provider !== provider) continue;
    if (!matchesSessionId(wantedIds, session.session_id)) continue;
    // Exact id wins over prefix match.
