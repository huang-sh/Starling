...
import { streamSessions } from "../lib/discovery.js";
import {
...
function formatAgentModelSummary(
...
export function registerProjectCommand(program: Command): void {
  const project = new Command("project")
    .alias("prj")
    .description("Manage projects — aggregate sessions by project directory");

  project
    .command("list")
    .alias("ls")
    .description("List all projects with session statistics")
    .option("-a, --agent <agent>", "filter by agent: claude | codex")
    .option("-n, --limit <number>", "max projects to show", "100")
    .option("--all", "show all projects")
    .option("--refresh-index", "rebuild ~/.starling/session-index.json before listing")
    .option("--no-index", "scan session files instead of using ~/.starling/session-index.json")
    .option("--json", "output as JSON")
    .action(
      async (opts: { agent?: string; limit?: string; all?: boolean; refreshIndex?: boolean; index?: boolean; json?: boolean }) => {
        const provider = opts.agent as Provider | undefined;
        const projectLimit = opts.all ? undefined : parseLimit(opts.limit);
        const scanLimit = opts.index === false ? projectLimit : undefined;
  ...
  project
    .command("show <path>")
    .description("Show project details and session list")
    .option("-a, --agent <agent>", "filter by agent: claude | codex")
    .option("--refresh-index", "rebuild ~/.starling/session-index.json before showing")
    .option("--no-index", "scan session files instead of using ~/.starling/session-index.json")
    .option("--json", "output as JSON")
    .action(
      async (path: string, opts: { agent?: string; refreshIndex?: boolean; index?: boolean; json?: boolean }) => {
        ...
        // Sort sessions by modified_at descending, show up to 20
