...
## CLI 方式：用 `starling run --setting`

等价的命令行方式是：

```bash
starling run --setting kimi claude
```

这里的含义是：

- `starling run`：由 Starling 启动 agent。
- `--setting kimi`：使用 `~/.starling/settings/claude/kimi.json` 这个 Starling 模型配置。
...
```bash
starling run --setting kimi -c paper-review --title "Kimi review" claude
```
...
```bash
starling run --setting kimi claude
starling run --setting kimi -c paper-review claude
starling run --setting kimi --catalog paper-review --title "Benchmark run" claude
starling run --cwd /data20T/dev/project --setting kimi claude
```

如果不指定 `--setting`，Starling 会使用 Claude Code 的系统默认配置：
...
```bash
starling run --setting kimi claude -p "总结这个项目的结构"
```

跳过权限确认：

```bash
starling run --setting kimi claude --dangerously-skip-permissions
```
...
```bash
starling run --setting kimi claude
starling run --setting ds claude
starling run --setting glm-5.2 claude
```
...
starling run --setting kimi --catalog paper-review claude
starling run --setting ds --catalog paper-review claude
starling run --setting glm-5.2 --catalog paper-review claude
```
...
```bash
starling run --setting kimi --catalog paper-review claude -p "审阅这篇论文项目"
```
...
```bash
starling run --setting ds --catalog paper-review claude -p "审阅这篇论文项目"
```
